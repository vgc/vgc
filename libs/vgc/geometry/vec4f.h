...
public:
    ...
    ///
    constexpr Vec4f() : data_{0, 0, 0, 0} {}


    /// Creates a `Vec4f` initialized with the given `x`, `y`, `z`, `w` coordinates.
    ///
    constexpr Vec4f(float x, float y, float z, float w) : data_{x, y, z, w} {}
    ...
    explicit constexpr Vec4f(const TVec4& other)
...
private:
    ...
    {
        const Vec4f& a = *this;
        return Vec4f(core::internal::infdiff(a[0], b[0]),
