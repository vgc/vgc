...
public:
    ...
    ///
    constexpr Rect2d(const Vec2d& pMin, const Vec2d& pMax) noexcept
        : pMin_(pMin)
        , pMax_(pMax) {
    }
    ...
    /// after this constructor.
    ///
    constexpr Rect2d(double xMin, double yMin, double xMax, double yMax) noexcept
        : pMin_(xMin, yMin)
        , pMax_(xMax, yMax) {
    }
    ...
    static constexpr Rect2d fromPositionSize(
        const Vec2d& position, const Vec2d& size) noexcept {
    ...
    ///
    static constexpr Rect2d fromPositionSize(
        const Vec2d& position, double width, double height) noexcept {
    ...
    ///
    static constexpr Rect2d fromPositionSize(
        double x, double y, const Vec2d& size) noexcept {
    ...
    /// are implicitly convertible to `Vec2d`.
    ///
    template<typename Range, VGC_REQUIRES(core::isInputRange<Range>)>
    static constexpr Rect2d computeBoundingBox(const Range& points) {
        Rect2d res = Rect2d::empty;
    ...
    static constexpr Rect2d computeBoundingBox(const Range& range, UnaryOp getPoint) {
        Rect2d res = Rect2d::empty;
    ...
    /// geometry::Vec2d topLeft = rect.corner(0, 0);
    /// geometry::Vec2d topRight = rect.corner(1, 0);
    /// geometry::Vec2d bottomRight = rect.corner(1, 1);
    /// geometry::Vec2d bottomLeft = rect.corner(0, 1);
    /// ```
    ///
    constexpr geometry::Vec2d corner(Int xIndex, Int yIndex) const {
    ...
    /// ```cpp
    /// // Example assuming the Y axis is pointing down
    /// geometry::Vec2d topLeft = rect.corner(0);
    /// geometry::Vec2d topRight = rect.corner(1);
    /// geometry::Vec2d bottomRight = rect.corner(2);
    /// geometry::Vec2d bottomLeft = rect.corner(3);
    /// ```
    ///
    constexpr geometry::Vec2d corner(Int index) const {
        switch (index) {
        case 0:
            return geometry::Vec2d(xMin(), yMin());
        case 1:
            return geometry::Vec2d(xMax(), yMin());
        case 2:
            return geometry::Vec2d(xMax(), yMax());
        default:
            return geometry::Vec2d(xMin(), yMax());
        }
    }
    ...
    bool intersectsSegment(
        const Vec2d& p0, const Vec2d& p1) const {
