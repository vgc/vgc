...
public:
    ...
    /// Creates a Mat3f initialized with the given row vectors.
    ///
    constexpr Mat3f(
        const Vec3f& v0,
        const Vec3f& v1,
    ...
    /// Creates a `Mat3f` from a `Mat<2, T>` matrix, assuming the given
    ...
    static constexpr Mat3f fromLinear(const TMat2& other) noexcept {
    ...
    /// Creates a `Mat3f` from a `Mat<2, T>` matrix, assuming the given matrix
    ...
    ///
    template<typename TMat2, VGC_REQUIRES(isMat<TMat2> && TMat2::dimension == 2)>
    static constexpr Mat3f fromTransform(const TMat2& other) noexcept {
        float a = static_cast<float>(other(0, 0));
        float b = static_cast<float>(other(0, 1));
        float c = static_cast<float>(other(1, 0));
        float d = static_cast<float>(other(1, 1));
    ...
    /// Accesses the component of the Mat3f at the `i`-th row and `j`-th column.
    ///
    const float& operator()(Int i, Int j) const {
        return data_[j][i];
    }

    /// Mutates the component of the Mat3f at the `i`-th row and `j`-th column.
    ///
    ...
    /// Returns the result of transforming the given `Vec2f` by the linear part
    /// of this `Mat3f` interpreted as a 2D projective transformation. That is,
    /// this ignores the translation and projective components, and only
    /// applies the rotation, scale, and shear components.
    /// 
    /// This is equivalent to multiplying this matrix by `Vec3f(x, y, 0)` and
...
/// Reads a `Mat3f` from the input stream, and stores it in the given output
