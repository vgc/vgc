...
public:
    ...
    /// 
    Vec2f clamp(const Vec2f& p) const {
        return {
    ...
    /// to this rectangle.
    /// 
    Rect2f clamp(const Rect2f& other) const {
        return Rect2f(clamp(other.pMin()), clamp(other.pMax()));
    }
    ...
    bool intersectsSegment(
        const Vec2f& p0, const Vec2f& p1) const {
