...
public:
    ...
    ///
    constexpr Range1f(float pMin, float pMax) noexcept
        : pMin_(pMin)
        , pMax_(pMax) {
    }
    ...
    /// to this range.
    /// 
    Range1f clamp(const Range1f& other) const {
        return Range1f(clamp(other.pMin()), clamp(other.pMax()));
    }
