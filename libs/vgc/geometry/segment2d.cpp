...
template<bool swap>
VGC_FORCE_INLINE double param(double t) {
    if constexpr (swap) {
        return 1 - t;
    }
    else {
...
VGC_FORCE_INLINE Segment2dIntersection pointInter(const Vec2d& p, double t1, double t2) {
    if constexpr (swapS) {
        return Segment2dIntersection(p, param<swap1>(t2), param<swap2>(t1));
    }
    else {
        return Segment2dIntersection(p, param<swap1>(t1), param<swap2>(t2));
    }
}
...
template<bool swapS, bool swap1, bool swap2>
VGC_FORCE_INLINE Segment2dIntersection
segInter(const Vec2d& p, const Vec2d& q, double s1, double t1, double s2, double t2) {
    if constexpr (swapS) {
...
template<bool swapS, bool swap1, bool swap2>
[[maybe_unused]] Segment2dIntersection intersectVerticalYOrdered_v1(
    const Vec2d& a1_,
    const Vec2d& b1_,
    const Vec2d& a2_,
    const Vec2d& b2_) {
...
// Assumes a1y <= a2y ("segments are y-ordered with each other")
//
// This version has less branching than v2 but more doubleing points operation
...
//
template<bool swapS, bool swap1, bool swap2>
[[maybe_unused]] Segment2dIntersection intersectVerticalYOrdered_v2(
    const Vec2d& a1_,
    const Vec2d& b1_,
    const Vec2d& a2_,
    const Vec2d& b2_) {
    ...
    if (b1 < a2) {
        ...
        // x------x     OR   x------x
        // x--------x          x------x
        double s1 = (a2 - a1) / (b1 - a1); // Guaranteed 0 if a1 == a2
        double t2 = (b1 - a2) / (b2 - a2); // (!) NOT guaranteed 0 < t2 < 1 [1]
        return segInter<swapS, swap1, swap2>(a2_, b1_, s1, 1, 0, t2);
    }
    else {
        ...
        // x------x        x----x            x----x          x--x
        double s1 = (a2 - a1) / (b1 - a1); // Guaranteed 0 if a1 == a2
        double t1 = (b2 - a1) / (b1 - a1); // Guaranteed 1 if b1 == b2
        return segInter<swapS, swap1, swap2>(a2_, b2_, s1, t1, 0, 1);
...
//
template<bool swapS, bool swap1, bool swap2>
[[maybe_unused]] Segment2dIntersection intersectCollinearXOrdered_(
    const Vec2d& a1_,
    const Vec2d& b1_,
    const Vec2d& a2_,
    const Vec2d& b2_) {
...
// [1] Unfortunately, due to how doubleing points work, then with:
//       a < x < b
//       t = (x - a) / (b - a)
...
Segment2dIntersection intersectVerticalYOrdered(
    const Vec2d& a1,
    const Vec2d& b1,
    const Vec2d& a2,
    const Vec2d& b2) {
...
intersectVerticalYOrderedWithPoint(const Vec2d& a1_, const Vec2d& b1_, const Vec2d& a2_) {
...
Segment2dIntersection
intersectVertical(const Vec2d& a1, const Vec2d& b1, const Vec2d& a2, const Vec2d& b2) {
...
template<bool swapS, bool swap1, bool swap2>
Segment2dIntersection
intersectXOrdered_(const Vec2d& a1, const Vec2d& b1, const Vec2d& a2, const Vec2d& b2) {
    ...
    Vec2d a1b2 = b2 - a1;
    Vec2d a2b1 = b1 - a2;
    double a1Side = -a2b2.det(a1a2); // == a2b2.det(a2a1)
    double b1Side = a2b2.det(a2b1);
    double a2Side = a1b1.det(a1a2);
    double b2Side = a1b1.det(a1b2);

    // Convert to sign since multiplying two non-zero doubles may give zero.
    ...
    if (s1Cross > 0 || s2Cross > 0) {
        ...
        //
        double delta = a1b1.det(a2b2); // depends on all four points, see [2]
        if (std::abs(delta) > 0) {
            double t1 = a1Side / delta;
            double t2 = -a2Side / delta;
        ...
        if (numCollinears == 1) {
            ...
            if (a2Sign == 0) {
                ...
                //
                double t1 = (a2.x() - a1.x()) / (b1.x() - a1.x()); // b2-independent
            ...
            if (a1Sign == 0 && b1Sign == 0) {
                ...
                //
                double d2 = a1a2.squaredLength(); // 0 if a1 == a2
                double delta = a1b1.det(a2b2);    // 0 if a1b1 parallel to a2b2
                if (d2 <= std::abs(delta)) {
                    return pointInter<swapS, swap1, swap2>(a1, 0, 0);
                }
                else {
                ...
                // (4 configurations of sign values)
                // Similar as [4]
                double d2 = a1b2.squaredLength();
                double delta = a1b1.det(a2b2);
                if (d2 <= std::abs(delta)) {
...
template<bool swap1, bool swap2>
Segment2dIntersection
intersectXOrdered(const Vec2d& a1, const Vec2d& b1, const Vec2d& a2, const Vec2d& b2) {
    if (a2.x() < a1.x()) {
...
template<bool swapS, bool swap1, bool swap2>
Segment2dIntersection
intersectXOrderedWithPoint(const Vec2d& a1, const Vec2d& b1, const Vec2d& a2) {
    Vec2d a1b1 = b1 - a1;
    Vec2d a1a2 = a2 - a1;
    double a2Side = a1b1.det(a1a2);
    if (a2Side == 0) {
        double t1 = (a2.x() - a1.x()) / (b1.x() - a1.x());
        return pointInter<swapS, swap1, swap2>(a2, t1, 0);
    }
    else {
        return {};
...
// Assumes a2y < b2y
template<bool swapS, bool swap1, bool swap2>
Segment2dIntersection intersectXOrderedWithVerticalYOrdered(
    const Vec2d& a1,
    const Vec2d& b1,
    const Vec2d& a2,
    const Vec2d& b2) {
    ...
    double t1 = (a2.x() - a1.x()) / (b1.x() - a1.x()); // Guaranteed 0 <= t1 <= 1
    if (s2Cross < 0) {
        Vec2d p = core::fastLerp(a1, b1, t1);
        double t2 = (p.y() - a2y) / (b2y - a2y);
        t2 = core::clamp(t2, 0, 1);
        return pointInter<swapS, swap1, swap2>(p, t1, t2);
    }
    else { // s2Cross == 0
...
// Assumes a1x < b1x and a2x == b2x
template<bool swapS, bool swap1, bool swap2>
Segment2dIntersection intersectXOrderedWithVertical(
    const Vec2d& a1,
    const Vec2d& b1,
    const Vec2d& a2,
    const Vec2d& b2) {
