...
public:
    ...
    ///
    constexpr Rect2x(const Vec2x& pMin, const Vec2x& pMax) noexcept
        : pMin_(pMin)
        , pMax_(pMax) {
    }
    ...
    /// after this constructor.
    ///
    constexpr Rect2x(float xMin, float yMin, float xMax, float yMax) noexcept
        : pMin_(xMin, yMin)
        , pMax_(xMax, yMax) {
    }
    ...
    static constexpr Rect2x fromPositionSize(
        const Vec2x& position, const Vec2x& size) noexcept {
    ...
    ///
    static constexpr Rect2x fromPositionSize(
        const Vec2x& position, float width, float height) noexcept {
    ...
    ///
    static constexpr Rect2x fromPositionSize(
        float x, float y, const Vec2x& size) noexcept {
    ...
    /// are implicitly convertible to `Vec2x`.
    ///
    template<typename Range, VGC_REQUIRES(core::isInputRange<Range>)>
    static constexpr Rect2x computeBoundingBox(const Range& points) {
        Rect2x res = Rect2x::empty;
    ...
    static constexpr Rect2x computeBoundingBox(const Range& range, UnaryOp getPoint) {
        Rect2x res = Rect2x::empty;
    ...
    /// geometry::Vec2x topLeft = rect.corner(0, 0);
    /// geometry::Vec2x topRight = rect.corner(1, 0);
    /// geometry::Vec2x bottomRight = rect.corner(1, 1);
    ...
    /// ```cpp
    /// // Example assuming the Y axis is pointing down
    /// geometry::Vec2x topLeft = rect.corner(0);
    /// geometry::Vec2x topRight = rect.corner(1);
    /// geometry::Vec2x bottomRight = rect.corner(2);
    ...
    constexpr geometry::Vec2x corner(Int index) const {
        ...
        case 1:
            return geometry::Vec2x(xMax(), yMin());
        case 2:
            return geometry::Vec2x(xMax(), yMax());
        default:
            return geometry::Vec2x(xMin(), yMax());
        }
    }
    ...
    /// 
    Vec2x clamp(const Vec2x& p) const {
        return {
    ...
    /// to this rectangle.
    /// 
    Rect2x clamp(const Rect2x& other) const {
        return Rect2x(clamp(other.pMin()), clamp(other.pMax()));
    }
