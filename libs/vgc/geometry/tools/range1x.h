...
#ifndef VGC_GEOMETRY_RANGE1X_H
#define VGC_GEOMETRY_RANGE1X_H
...
/// \class vgc::core::Range1x
/// \brief 2D range using %scalar-type-description%.
///
/// This class represents a 1D range using %scalar-type-description%.
///
...
///
/// Alternatively, you can create a `Range1x` by providing its `position` and
/// `size`:
///
/// - `Range1x::fromPositionSize(float position, float size)`
///
/// If `size < 0`, then the range is considered empty.
///
/// If you want to make sure that a range isn't empty, you can create a
/// `Range1x` then call `normalize()` or `normalized()`, which swaps the min and
...
public:
    ...
    ///
    Range1x(core::NoInit) noexcept {
    }
    VGC_WARNING_POP

    /// Creates a zero-initialized `Range1x`.
    ///
    /// This is equivalent to `Range1x(0, 0)`.
    ///
    constexpr Range1x() noexcept
        : pMin_()
        , pMax_() {
    }

    /// Creates a `Range1x` defined by the two points `pMin` and `pMax`.
    ///
    ...
    ///
    constexpr Range1x(float pMin, float pMax) noexcept
        : pMin_(pMin)
        , pMax_(pMax) {
    }


    /// Creates a `Range1x` from a `position` and `size`.
    ///
    /// This is equivalent to `Range1x(position, position + size)`.
    ///
    /// If `size < 0`, then the range is considered empty.
    ///
    /// You can ensure that the range isn't empty by calling `normalize()`
    ...
    /// pMax()`.
    ///
    constexpr Range1x normalized() {
        std::pair<float, float> p = std::minmax(pMin_, pMax_);
        return Range1x(p.first, p.second);
    }
    ...
    ///
    friend constexpr bool operator==(const Range1x& r1, const Range1x& r2) {
    ...
    /// instead.
    ///
    bool isClose(const Range1x& other, float relTol = 1e-5f, float absTol = 0.0f) const {
    ...
    ///
    bool isNear(const Range1x& other, float absTol) const {
    ...
    /// to this range.
    /// 
    Range1x clamp(const Range1x& other) const {
        return Range1x(clamp(other.pMin()), clamp(other.pMax()));
    }
    ...
    /// Range1x r1(0, 1);
    /// Range1x r2(2, 3);
    /// Range1x r3 = r1.unitedWith(r2);            // == Range1x(0, 3)
    /// Range1x r4 = r1.unitedWith(Range1x.empty); // == Range1x(0, 1)
    /// ```
    ///
    ...
    /// Range1x r3 = r1.unitedWith(r2); // == Range1x(0, 2) (!)
    /// ```
    ///
    ...
    /// Range1x s2 = r1.unitedWith(r2); // == Range1x(0, 2)
    /// Range1x s3 = r1.unitedWith(r3); // == Range1x(0, 2)
    /// Range1x s4 = r1.unitedWith(r4); // == Range1x(0, 2)
    /// ```
    ///
    /// This behavior is intended and will not change in future versions, so
    /// you can rely on it for your algorithms.
    ///
    constexpr Range1x unitedWith(const Range1x& other) const {
        return Range1x(
    ...
    /// and the given `point`.
    ///
    /// This is equivalent to `unitedWith(Range1x(point))`.
    ///
    /// See `unitedWith(const Range1x&)` for more details, in particular about
    /// how it handles empty ranges: uniting an empty range with a
    /// point may result in a range larger than just the point.
    ///
    /// However, uniting `Range1x::empty` with a point always results in the
    /// range reduced to just the point.
    ///
    constexpr Range1x unitedWith(float point) const {
        return Range1x(
            (std::min)(pMin_, point),
            (std::max)(pMax_, point));
    }
    ...
    /// of this range).
    ///
    constexpr Range1x& uniteWith(const Range1x& other) {
    ...
    /// and the given `point`.
    ///
    /// This is equivalent to `uniteWith(Range1x(point))`.
    ///
    /// See `unitedWith(const Range1x&)` for more details, in particular about
    /// how it handles empty ranges: uniting an empty range with a
    /// point may result in a range larger than just the point.
    ...
    /// range reduced to just the point.
    ///
    constexpr Range1x& uniteWith(float other) {
        *this = unitedWith(other);
        return *this;
    }
    ...
    /// Range1x r2(2, 4);
    /// Range1x r3(5, 6);
    /// Range1x r4(2, 1); // (empty)
    ///
    /// Range1x s2 = r1.intersectedWith(r2);            // == Range1x(2, 3)
    /// Range1x s3 = r1.intersectedWith(r3);            // == Range1x(5, 3)  (empty)
    /// Range1x s4 = r1.intersectedWith(r4);            // == Range1x(2, 1)  (empty)
    /// Range1x s5 = r1.intersectedWith(Range1x.empty); // == Range1x.empty        (empty)
    /// ```
    ///
    ...
    ///
    constexpr Range1x intersectedWith(const Range1x& other) const {
        return Range1x(
    ...
    ///
    constexpr Range1x& intersectWith(const Range1x& other) {
    ...
    /// with `Range1x::empty`.
    ///
    constexpr bool intersects(const Range1x& other) const {
    ...
    /// with `Range1x::empty`.
    ///
    constexpr bool contains(const Range1x& other) const {
...
inline constexpr Range1x Range1x::empty = Range1x(
    core::infinity<float>, -core::infinity<float>);

/// Alias for `vgc::core::Array<vgc::core::Range1x>`.
///
using Range1xArray = core::Array<Range1x>;

/// Overloads `setZero(T& x)`.
///
/// \sa `vgc::core::zero<T>()`
///
inline void setZero(Range1x& r) {
    r = Range1x();
}
...
void write(OStream& out, const Range1x& r) {
    write(out, '(', r.pMin(), ", ", r.pMax(), ')');
}

/// Reads a `Range1x` from the input stream, and stores it in the output
/// parameter `r`. Leading whitespaces are allowed. Raises `ParseError` if the
/// stream does not start with a valid string representation of a `Range1x`.
