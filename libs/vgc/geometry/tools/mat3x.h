...
public:
    ...
    /// Creates a Mat3x initialized with the given row vectors.
    ///
    constexpr Mat3x(
        const Vec3x& v0,
        const Vec3x& v1,
        const Vec3x& v2) noexcept
    ...
    /// Returns the result of transforming the given `Vec2x` by the linear part
    /// of this `Mat3x` interpreted as a 2D projective transformation. That is,
    /// this ignores the translation and projective components, and only
    /// applies the rotation, scale, and shear components.
    /// 
    /// This is equivalent to multiplying this matrix by `Vec3x(x, y, 0)` and
...
/// Alias for `vgc::core::SharedConstArray<vgc::geometry::Mat3x>`.
///
using SharedConstMat3xArray = core::SharedConstArray<Mat3x>;
...
/// Reads a `Mat3x` from the input stream, and stores it in the given output
