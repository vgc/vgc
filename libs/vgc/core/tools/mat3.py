...
# This script generates all mat3.h/.cpp files. Usage:
#
# ./mat4.py
#
...
// Instead, edit ./tools/{}, and re-run ./tools/mat3.py.
'''
...
def generateFile(templateFile, outputFile, tchar, tname, tdesc, relTol):
    ...
    s = s.replace("MAT2X", "MAT2" + tchar.upper())
    s = s.replace("mat2x", "mat2" + tchar)
    s = s.replace("Mat2x", "Mat2" + tchar)
    s = s.replace("MAT3X", "MAT3" + tchar.upper())
    s = s.replace("mat3x", "mat3" + tchar)
    s = s.replace("Mat3x", "Mat3" + tchar)
    ...
    s = s.replace('"mat2.h"', "<vgc/core/mat2" + tchar + ".h>")
    s = s.replace('"mat3.h"', "<vgc/core/mat3" + tchar + ".h>")
    ...
    # Autogen info
    s = s.replace(autogenTemplateInfo,
                  autogenOutputInfo.format(templateFile.name))

    outputFile.write_text(s)


def generateVariant(libDir, toolsDir, tchar, tname, tdesc, relTol):
    generateFile(toolsDir / "mat3.h", libDir / ("mat3"+tchar+".h"), tchar, tname, tdesc, relTol)
    generateFile(toolsDir / "mat3.cpp", libDir / ("mat3"+tchar+".cpp"), tchar, tname, tdesc, relTol)
