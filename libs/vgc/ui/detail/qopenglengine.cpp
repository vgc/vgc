...
void QglEngine::initContext_() {
    ...
    if (hasAnisotropicFilteringSupport_) {
        api_->glGetFloatv(GL_MAX_TEXTURE_MAX_ANISOTROPY, &maxTextureMaxAnisotropy);
    }
}
...
void QglEngine::initSamplerState_(SamplerState* aState) {
    ...
    api_->glSamplerParameterf(object, GL_TEXTURE_MAX_ANISOTROPY, state->maxAnisotropyGL_);
